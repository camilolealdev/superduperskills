...
offline (e.g. nightly), it reviews their real past Codex sessions,
...
- **Codex Dreams** — offline consolidation that reads past sessions and rebuilds
...
1. **Harvest** — read `~/.Codex/projects/*/<session>.jsonl` + `~/.Codex/history.jsonl` (READ-ONLY) → session digests.
...
- Add `--backend Codex` or `--backend codex` to spend the user's real budget for genuine improvement.
...
| `--backend mock\|Codex\|codex\|copilot` | mock | Replay backend (mock = no API spend) |
| `--model NAME` | backend default | Override the model used for replay |
| `--source Codex\|codex\|auto` | Codex | Transcript source |
