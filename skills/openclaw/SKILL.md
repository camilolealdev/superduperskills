...
If this machine also runs Codex with Codex-mem installed, the worker may already be running:
...
#### If Codex has Codex-mem installed

If Codex-mem is installed as a Codex plugin (at `~/.Codex/plugins/marketplaces/thedotmack/`), start the worker from that installation:
...
#### If there's no Codex installation
...
| Worker started from Codex install but not responding | Check `cd ~/.Codex/plugins/marketplaces/thedotmack && npm run worker:status`. May need `npm run worker:restart`. |
