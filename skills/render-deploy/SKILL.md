...
2) Add the MCP server with Codex (replace `<YOUR_API_KEY>`):
```bash
Codex mcp add --transport http render https://mcp.render.com/mcp --header "Authorization: Bearer <YOUR_API_KEY>"
```

3) Restart Codex, then retry `list_services()`.
