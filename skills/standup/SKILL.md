...
The room is one shared file (default `~/.Codex-mem/STANDUP.md`): YAML front
...
> `~/.Codex-mem/STANDUP.md`; speak with `node "<skill-dir>/standup.mjs" post …`,
...
```
Defaults: agent = git branch, file = `~/.Codex-mem/STANDUP.md`. Every write is
atomically locked.
