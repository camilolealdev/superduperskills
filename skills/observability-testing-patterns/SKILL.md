...
Dashboard screenshot validation and alert-UI verification go through the **qe-browser** fleet skill (`.Codex/skills/qe-browser/`). Vibium is installed by `aqe init`. Typical dashboard regression workflow:
...
node .Codex/skills/qe-browser/scripts/visual-diff.js --name "grafana-api-latency"
```
