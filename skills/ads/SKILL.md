...
| Install, update, or uninstall Codex Ads safely | `/ads setup` for install; `/ads validate` for uninstall |
...
Write each run beneath `.Codex-ads/runs/<run-id>/` with a manifest and atomic
...
- Uninstall only exact files and directories recorded by Codex Ads' ownership
...
never hardcode `~/.Codex`. Load only what the request needs:
