...
GITHUB_URLS = {
    ...
    'ci-security-scanning-with-strix': 'https://github.com/usestrix/strix',
    'penetration-testing-with-strix': 'https://github.com/usestrix/strix',
    'fix-security-vulnerabilities-with-strix': 'https://github.com/usestrix/strix',
    'managed-pentesting-with-strix': 'https://github.com/usestrix/strix',
    'api-security-testing': 'https://github.com/usestrix/strix',
    'application-security-testing': 'https://github.com/usestrix/strix',
    'find-security-vulnerabilities-in-code': 'https://github.com/usestrix/strix',
    'owasp-top-10-testing': 'https://github.com/usestrix/strix',
    # === Spring Boot Security & Enterprise (ECC) ===
    'springboot-security': 'https://github.com/affaan-m/everything-claude-code',
    'springboot-patterns': 'https://github.com/affaan-m/everything-claude-code',
    'springboot-tdd': 'https://github.com/affaan-m/everything-claude-code',
    'springboot-verification': 'https://github.com/affaan-m/everything-claude-code',
    'spring-boot-testing': 'https://github.com/affaan-m/everything-claude-code',
    'spring-boot-engineer': 'https://github.com/affaan-m/everything-claude-code',
    'agentshield': 'https://github.com/affaan-m/everything-claude-code',
    'ecc-guide': 'https://github.com/affaan-m/everything-claude-code',
    'ecc-recipes': 'https://github.com/affaan-m/everything-claude-code',
    'ecc-tools-cost-audit': 'https://github.com/affaan-m/everything-claude-code',
    # === OWASP & Threat Modeling ===
    'owasp-security': 'https://github.com/behisecc/claude-security-skills',
    'threat-model-analyst': 'https://github.com/behisecc/claude-security-skills',
}
...
def get_git_remote(dir_path):
    ...
    if dir_path in _REMOTE_CACHE:
        return _REMOTE_CACHE[dir_path]
    res = ''
    try:
        r = subprocess.run(['git', 'remote', 'get-url', 'origin'],
                           cwd=dir_path, capture_output=True, text=True, timeout=5)
        if r.returncode == 0:
            res = r.stdout.strip().replace('.git', '').rstrip('/')
    except:
        pass
    _REMOTE_CACHE[dir_path] = res
    return res
...
for repo, base_dir in SKILL_DIRS:
    ...
    for fp in Path(base_dir).rglob('SKILL.md'):
        ...
        if take_it:
            ...
            github_url = GITHUB_URLS.get(name, '')
            if not github_url:
                if repo == 'agents':
...
for name, data in skills.items():
    ...
    if os.path.abspath(data['source_dir']) == os.path.abspath(dest_dir):
        continue
    if _is_link_or_junction(dest_dir):
        ...
        src_skill = os.path.join(data['source_dir'], 'SKILL.md')
        dest_skill = os.path.join(dest_dir, 'SKILL.md')
        needs_update = False
        if not os.path.isfile(dest_skill):
            needs_update = True
        else:
            try:
                if os.path.getmtime(src_skill) > os.path.getmtime(dest_skill) or os.path.getsize(src_skill) != os.path.getsize(dest_skill):
                    needs_update = True
            except OSError:
                needs_update = True
        if needs_update:
